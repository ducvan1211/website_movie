                        ...
                        <form action="{{ route('category.update', $cat->id) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror"
                                    id="title" value="{{ $cat->title }}" name="title">
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="desc" id="desc" cols="30" rows="5"
                                    class="form-control @error('desc') is-invalid @enderror">{{ $cat->desc }}</textarea>
                                @error('desc')
                                    <small class="form-text text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-select" name="status" id="status">
                                    <option {{ $cat->status === '1' ? 'selected' : '' }} value="1">Hiển thị</option>
                                    <option {{ $cat->status === '0' ? 'selected' : '' }} value="0">Không hiển thị
                                    </option>
                                </select>
                            </div>
